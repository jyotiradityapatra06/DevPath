...
VALID_INTERVIEW_PREPARATION = {
    ...
    "focus_areas": ["FastAPI", "Database Design"],
    "confidence_score": 85,
}
...
class MockProvider(AIProvider):
    def __init__(self, response: str | None = None) -> None:
        self.response = response or json.dumps(VALID_INTERVIEW_PREPARATION)
...
def test_endpoint_requires_authentication(client: TestClient) -> None:
    ...
    response = client.post("/api/v1/interview/prepare")

    assert response.status_code == 401


def test_successful_interview_generation(client: TestClient) -> None:
    headers = create_user(
        client, "Ada", "ada-interview@example.com", "Backend Engineer"
    )
    provider = MockProvider()
    override_provider(client, provider)

    response = client.post("/api/v1/interview/prepare", headers=headers)

    assert response.status_code == 200
    assert response.json() == VALID_INTERVIEW_PREPARATION
...
def test_user_data_isolation(client: TestClient) -> None:
    ada_headers = create_user(
        client,
        "Ada",
        "ada-interview-isolation@example.com",
        "Backend Engineer",
    )
    create_user(
        client,
        "Lin",
        "lin-interview-isolation@example.com",
        "Data Scientist",
    )
    provider = MockProvider()
    override_provider(client, provider)

    response = client.post("/api/v1/interview/prepare", headers=ada_headers)
...
def test_provider_failure_returns_bad_gateway(client: TestClient) -> None:
    headers = create_user(
        client, "Ada", "ada-interview-failure@example.com", "Backend Engineer"
    )
...
def test_invalid_ai_response_returns_bad_gateway(client: TestClient) -> None:
    headers = create_user(
        client, "Ada", "ada-interview-invalid@example.com", "Backend Engineer"
    )

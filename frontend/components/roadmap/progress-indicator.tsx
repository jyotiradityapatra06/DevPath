  ...
  return (
    <div className="min-w-0" aria-label={`${label}: ${value}%`}>
      ...
      <div className="h-1.5 overflow-hidden rounded-full bg-white/[0.07]">
        <motion.div initial={{ scaleX: 0 }} animate={{ scaleX: value / 100 }} transition={{ duration: 0.9, ease: "easeOut" }} className="h-full w-full origin-left rounded-full bg-gradient-to-r from-[#8B5CF6] via-[#A78BFA] to-[#10B981]" />
      </div>
    </div>
  )
}

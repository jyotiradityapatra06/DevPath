    ...
    <Dialog.Root open={open} onOpenChange={setOpen}>
      <Dialog.Portal>
        ...
        <Dialog.Content className="fixed inset-y-0 left-0 z-50 w-[min(86vw,20rem)] border-r border-sidebar-border bg-sidebar p-5 text-sidebar-foreground shadow-2xl focus:outline-none data-[state=open]:animate-in data-[state=open]:slide-in-from-left">
          ...
          <nav className="mt-10"><Dialog.Close asChild><Link href="/app" className="flex h-11 items-center gap-3 rounded-xl bg-white/10 px-3 text-sm font-medium text-white"><LayoutGrid className="size-4" />Workspace</Link></Dialog.Close></nav>
        </Dialog.Content>
      </Dialog.Portal>
    </Dialog.Root>
  )

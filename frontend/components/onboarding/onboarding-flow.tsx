...
import { mapBackendRolesToCards } from "@/features/onboarding/config/role-mapping"
import { useCompleteOnboarding } from "@/features/onboarding/hooks/use-complete-onboarding"
import { useRoles } from "@/features/onboarding/hooks/use-roles"
...
export function OnboardingFlow() {
  ...
  const { user } = useAuth()
  const { data: backendRoles = [] } = useRoles()
  ...
  const careerGoalCards = useMemo(() => {
    if (backendRoles.length > 0) {
      return mapBackendRolesToCards(backendRoles).map((card) => ({
        id: card.id,
    ...
    return fallbackCareerGoals.map((g) => ({ ...g, id: undefined, canonicalTitle: g.label, description: undefined }))
  }, [backendRoles])
  ...
  async function handleContinue() {
    ...
    try {
      if (step === 1) {
        await profileMutation.mutateAsync({ careerIntent: state.careerIntent })
      }
      if (step === 2) {
    ...
    <main className="dark relative min-h-screen overflow-hidden bg-[#09090B] px-4 py-5 text-[#FAFAFA] sm:px-6 sm:py-8">
      ...
      <div className="relative mx-auto flex min-h-[calc(100vh-2.5rem)] max-w-5xl flex-col sm:min-h-[calc(100vh-4rem)]">
        <header className="flex items-center justify-between">
          ...
          <p className="hidden text-xs text-[#71717A] sm:block">Private, personalized, built around you.</p>
        </header>
          ...
          <div className="mx-auto max-w-3xl">
            ...
            <div className="mt-8 min-h-[30rem] rounded-[1.75rem] border border-white/[0.09] bg-[#111113]/90 p-5 shadow-[0_36px_110px_-45px_rgba(0,0,0,0.9)] backdrop-blur-xl sm:mt-10 sm:p-8 lg:p-10">
              <AnimatePresence mode="wait">
                <div key={step}>
                  {step === 1 && (
                    <StepContainer eyebrow="Welcome to DevPath" title="Let's understand your career journey." description="DevPath will analyze your goals, skills, and experience to build your personalized career path.">
                      <div className="grid gap-3 sm:grid-cols-3">
                        {[{ icon: Target, label: "Your direction", color: "text-[#A78BFA]" }, { icon: BrainCircuit, label: "Your intelligence", color: "text-[#6EE7B7]" }, { icon: Sparkles, label: "Your path", color: "text-[#FBBF24]" }].map(({ icon: Icon, label, color }) => (
                          <div key={label} className="rounded-2xl border border-white/[0.07] bg-[#18181B] p-4">
                  ...
                  {step === 2 && (
                    <StepContainer eyebrow="Career goal" title="Where do you want to go?" description="Choose the role that best matches the direction you want to build toward.">
                      <div className="grid gap-3 sm:grid-cols-2">
                        {careerGoalCards.map((option) => (
                          <OptionCard
                            key={option.label}
                            {...option}
                            selected={state.targetRole === option.label || state.targetRoleId === option.id}
                            onSelect={() =>
                              setState((current) => ({
                                ...current,
                                targetRole: option.label,
                                targetRoleId: option.id,
                              }))
                            }
                          />
                        ))}
                  ...
                  {step === 3 && (
                    <StepContainer eyebrow="Experience level" title="Where are you starting from?" description="This helps DevPath shape recommendations at the right depth and pace.">
                      <div className="grid gap-3">
                        {experienceLevels.map((option) => (
                          <OptionCard
                            key={option.label}
                            {...option}
                            selected={state.experienceLevel === option.label}
                            onSelect={() => setState((current) => ({ ...current, experienceLevel: option.label }))}
                          />
                        ))}
                      </div>
                    </StepContainer>
                  ...
                  {step === 4 && (
                    <StepContainer eyebrow="Current skills" title="What do you already know?" description="Select the skills that represent your current toolkit. You can refine levels later.">
                      <SkillSelector
                        selected={state.skills}
                        selectedSkillIds={state.skillIds}
                        onChange={(skills, skillIds) => setState((current) => ({ ...current, skills, skillIds }))}
                      />
                    </StepContainer>
                  )}
                  {step === 5 && (
                    <StepContainer eyebrow="Learning preference" title="How do you learn best?" description="Choose the style that should influence how your growth path is presented.">
                      <div className="grid gap-3 sm:grid-cols-2">
                        {learningPreferences.map((option) => (
                          <OptionCard
                            key={option.label}
                            {...option}
                            selected={state.learningPreference === option.label}
                            onSelect={() => setState((current) => ({ ...current, learningPreference: option.label }))}
                          />
                        ))}
                      </div>
                    </StepContainer>
                  ...
                  {step === 6 && (
                    <StepContainer eyebrow="Profile complete" title="Your AI career profile is ready." description="DevPath now has the foundation it needs to shape a career experience around your direction.">
                      <div className="relative overflow-hidden rounded-3xl border border-[#10B981]/20 bg-[#10B981]/[0.06] p-6 text-center sm:p-8">
                        <motion.div initial={{ scale: 0.7, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} transition={{ delay: 0.1, type: "spring", stiffness: 180, damping: 14 }} className="mx-auto grid size-16 place-items-center rounded-2xl border border-[#10B981]/30 bg-[#10B981]/10 text-[#6EE7B7]">
                          <Check className="size-8" />
                        </motion.div>
                        <p className="mt-6 text-sm text-[#A1A1AA]">{state.targetRole} · {state.experienceLevel} · {state.skills.length} skills</p>
                        <button type="button" onClick={handleComplete} disabled={completeMutation.isPending} className="mt-7 inline-flex h-12 items-center gap-2 rounded-xl bg-[#8B5CF6] px-6 text-sm font-semibold text-white transition-all hover:-translate-y-0.5 hover:bg-[#9D74F7] hover:shadow-xl hover:shadow-[#8B5CF6]/20 disabled:pointer-events-none disabled:opacity-60">
                          {completeMutation.isPending ? <><LoaderCircle className="size-4 animate-spin" />Creating your AI career profile...</> : <>Enter DevPath <ArrowRight className="size-4" /></>}
                        </button>
                      </div>
                    </StepContainer>
                  )}
